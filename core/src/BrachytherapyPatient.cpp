...
std::list<BrachytherapySeedPosition> 
   ...
   const std::vector<boost::shared_ptr<BrachytherapySeedProxy> > &seeds ) const
{
  ...
  std::list<BrachytherapySeedPosition> potential_seed_positions;
  ...
  for( unsigned i = 0; i < seeds.size(); ++i )
  {
    ...
    createSeedPositions( potential_seed_positions,
			 seeds[i],
  ...
  std::list<BrachytherapySeedPosition>::const_iterator position, 
    test_position, end_position;
  position = potential_seed_positions.begin();
  end_position = potential_seed_positions.end();
	          ...
	          const boost::shared_ptr<BrachytherapySeedProxy> &seed ) const
{
  ...
  return getPotentialSeedPositions( seeds );
}
...
void BrachytherapyPatient::createSeedPositions( 
		         ...
		         const std::vector<double> &rectum_adjoint_data ) const
{
  ...
  for( unsigned j = 0; j < d_mesh_y_dim; ++j )
  {
	...
	for( unsigned slice = 0; slice < d_mesh_z_dim; ++slice )
	{
	  ...
	  {
	    double weight = 
	      (urethra_adjoint_data[mask_index]+margin_adjoint_data[mask_index]
	       +rectum_adjoint_data[mask_index])/
	      prostate_adjoint_data[mask_index];
	    
	    seed_positions.push_back( 
			BrachytherapySeedPosition(i, j, slice, weight, seed) );
	  }
	}
      }
    }
