...
private:
  ...
  advanced_shared_array<int> _urethra;
  advanced_shared_array<int> _rectum;
  advanced_shared_array<int> _margin;
  double _target_adjoint;
  double _urethra_adjoint;
  double _rectum_adjoint;
  double _margin_adjoint;
...
public:
  ...
  ratio(advanced_shared_array<double> current_dose,
	...
	double target_adjoint,
	double urethra_adjoint,
	double rectum_adjoint,
	double margin_adjoint,
  ...
  { _current_dose = current_dose;
    ...
    _rectum = rectum;
    _margin = margin;
    _target_adjoint = target_adjoint;
    _urethra_adjoint = urethra_adjoint;
    _rectum_adjoint = rectum_adjoint;
    _margin_adjoint = margin_adjoint;
  ...
  void update(double MaxTaDose, double MaxUrDose, double MaxReDose,
	      double MaxMaDose)
  {
    coverage = 0.0;
    double Wta=1.0, Wur=1.0, Wre=1.0, Wma=1.0;
    double tmp;
    int index;
	...
	for(int l=_seedcenters[0]-xindex, Fi=l+_meshdims[0], il=0; l<Fi; l++,
	  ...
	  if(_target[index] > 0){
	    if(_current_dose[index] < _Dp){
	      tmp < _Dp ? coverage += tmp - _current_dose[index] : 
		coverage += _Dp-_current_dose[index];
	    }
	    else if(_current_dose[index] < MaxTaDose && tmp > MaxTaDose){
	      //coverage -= tmp - MaxTaDose;
	      Wta += 1.0;
	    }
	    else if(_current_dose[index] > MaxTaDose){
	      //coverage -= tmp - _current_dose[index];
	      Wta += 1.0;
	    }
	  }
	  else if(_urethra[index] > 0){
	    if(_current_dose[index] < MaxUrDose && tmp > MaxUrDose)
	      //coverage -= tmp - MaxUrDose;
	      Wur += 1.0;
	    else if(_current_dose[index] > MaxUrDose)
	      //coverage -= tmp - _current_dose[index];
	      Wur += 1.0;
	  }
	  else if(_rectum[index] > 0){
	    if(_current_dose[index] < MaxReDose && tmp > MaxReDose)
	      //coverage -= tmp - MaxReDose;
	      Wre += 1.0;
	    else if(_current_dose[index] > MaxReDose)
	      //coverage -= tmp - _current_dose[index];
	      Wre += 1.0;
	  }
	  else if(_margin[index] > 0){
	    if(_current_dose[index] < MaxMaDose && tmp > MaxMaDose)
	      //coverage -= tmp - MaxMaDose;
	      Wma += 1.0;
	    else if(_current_dose[index] > MaxMaDose)
	      //coverage -= tmp - _current_dose[index];
	      Wma += 1.0;
	  }
	}
    ...
    value = (Wur*_urethra_adjoint+Wre*_rectum_adjoint+Wma*_margin_adjoint)/
      (1.0/Wta*_target_adjoint);
  ...
  ratio & operator= (const ratio& that){
	...
	_urethra = that._urethra;
	_rectum = that._rectum;
	_margin = that._margin;
...
int main(int argc, char** argv)
{
  ...
  //ideal max doses for different organs
  double MDur=std::numeric_limits<double>::infinity();
  double MDre=MDur, MDma=MDur, MDta=MDur;
...
//------------------CREATE ORGAN MESH IN MOAB-------------------------------//
  ...
  //  	(Wta*target_adjoint[j][i]);
      //adjoint_ratios[j][i] = (urethra_adjoint[j][i]*urethra_vol+
      //		      rectum_adjoint[j][i]*rectum_vol+
      //		      margin_adjoint[j][i]*margin_vol)/
      //(target_adjoint[j][i]*target_vol);
  //}
  //}
  ...
  for(int k=0; k<meshpts[2]; k++){
	...
	if(needle_template[tmpindex] == 1){
	  ...
	  if(use_all == true){
	    for(int a=0; a<angles; a++){
	      ratio tmpratio(currentdose,
			     ...
			     rectum,
			     margin,
			     target_adjoint[a][tmpindex],
			     urethra_adjoint[a][tmpindex],
			     rectum_adjoint[a][tmpindex],
			     margin_adjoint[a][tmpindex],
			     anisoseedcents,
			     anisoseedpts,
			     meshpts,
			     Dp,
	      ...
	      tmpratio.update(MDta,MDur,MDre,MDma);
	      templateratios.push_back(tmpratio);
	    }
	  }
	  else{
	    double minval = std::numeric_limits<double>::infinity();
	    double tmpval = std::numeric_limits<double>::infinity();
	    int tmpl=0;
	    for(int a=0; a<angles; a++){
	    ...
	    ratio tmpratio(currentdose,
			   ...
			   rectum,
			   margin,
			   target_adjoint[tmpl][tmpindex],
			   urethra_adjoint[tmpl][tmpindex],
			   rectum_adjoint[tmpl][tmpindex],
			   margin_adjoint[tmpl][tmpindex],
			   anisoseedcents,
			   anisoseedpts,
			   meshpts,
			   Dp,
	  ...
	  ratio tmpratio(currentdose,
			 ...
			 rectum,
			 margin,
			 target_adjoint[angles][tmpindex],
			 urethra_adjoint[angles][tmpindex],
			 rectum_adjoint[angles][tmpindex],
			 margin_adjoint[angles][tmpindex],
			 isoseedcents,
			 isoseedpts,
			 meshpts,
			 Dp,
  ...
  while((double)numV100/target_vol < 0.98 && templateratios.size() > 0){
	...
	(*sit).update(MDta,MDur,MDre,MDma);
	sit++;
      }
    }
